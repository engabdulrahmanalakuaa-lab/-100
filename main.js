...
function initializeDatabase() {
    ...
    // البيانات الافتراضية (إذا لم توجد شركة)
    ...
    if (companyCount.count === 0) {
        ...
        const accId = db.prepare("SELECT last_insert_rowid()").get()['last_insert_rowid()'];
        ...
        const cashId = db.prepare("SELECT last_insert_rowid()").get()['last_insert_rowid()'];
...
ipcMain.handle('get-company', () => {
    try {
        return db.prepare("SELECT * FROM companies LIMIT 1").get() || null;
    } catch (e) {
        return null;
    }
});
...
ipcMain.handle('create-order', (event, data) => {
    try {
        ...
        const info = db.prepare(`INSERT INTO orders (company_id, table_id, waiter_id, user_id, total, tax, total_with_tax, discount, payment_method, paid_amount, date, time, shift_id)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`)
            .run(company_id, table_id, waiter_id, user_id, total, tax || 0, total_with_tax || total, discount || 0, payment_method, paid_amount, today, time, shift_id);
...
ipcMain.handle('get-inventory-report', (event, { companyId }) => {
    try {
        return db.prepare("SELECT * FROM raw_materials WHERE company_id=?").all(companyId);
    } catch (e) {
        return [];
    }
});
...
// ========== تصدير PDF ==========
ipcMain.handle('export-pdf', (event, { content, title, userId }) => {
    try {
        const doc = {
            content: content,
...
console.log('✅ نظام تقنيات سوفت المطور جاهز (باستخدام better-sqlite3)');
